...
let packages =
  [ package "uri"; package "magic-mime"; package "letsencrypt"; package ~min:"0.8.7" "fmt" ]
...
let main =
  let keys = [ Key.abstract http_port; Key.abstract https_port; Key.abstract hostname ] in
  foreign
    ~packages
    ~keys
    "Dispatch.HTTPS"
    (pclock @-> resolver @-> conduit @-> kv_ro @-> time @-> http @-> job)
...
let () =
  ...
  register
    "mvgce"
    [ main $ default_posix_clock $ res_dns $ conduit $ data $ default_time $ https_srv ]
